...
def main():
    ...
    if login():
        ...
        rendimentos_totais, rendimentos_totais_inter, rendimentos_totais_nubank, rendimentos_totais_neon = investment_calculator.calcular_rendimentos(contributions)
        ...
        with col4:
            sl = rendimentos_totais_neon.query("Data == @today").iloc[0]['Saldo líquido']
            rl = rendimentos_totais_neon.query("Data == @today").iloc[0]['Rendimento líquido']
            sl = locale.currency(sl, grouping=True)
            rl = locale.currency(rl, grouping=True)
            st.write(f"<span style='color: #08AEF0; font-size: 1.5rem;'>Neon</span>", unsafe_allow_html=True)
            st.write(f"<h1 style='padding:0;'>{sl}</h1>", unsafe_allow_html=True)
            st.write(f"<h5 style='color: #08AEF0;'>↑ {rl}</h5>", unsafe_allow_html=True)
        ...
        with col2:
            opcao = st.selectbox('Selecione a conta', ['Total', 'Inter', 'Nubank', 'Neon'])
        ...
        if opcao == 'Total':
            ...
            rendimentos_totais_neon = rendimentos_totais_neon.map(formatar_numeros)
            st.table(rendimentos_totais_neon.set_index(rendimentos_totais.columns[0]))
